...
module.exports = {
  name: 'pneumonia',
  rule: '0 */10 * * * *',
  async tick(createdAt, app) {
    console.log(this.name, createdAt);
    const { jobBLL, pneumoniaBLL } = app.BLL;
    const _id = uuid.v4();
    const data = {
      _id,
      name: this.name,
      createdAt,
      status: constant.JOB_STATUS.PENDDING,
      duration: 0
    }
    await jobBLL.create(data);
    try {
      ...
      if (resp.status === 200) {
        ...
        const duration = dayjs.utc().tz(config.timezone).toDate().getTime() - dayjs.utc(createdAt).tz(config.timezone).toDate().getTime()
        if (result.errno === 0) {
          const at = result.data.pneumonia.upate_time;
          const date = at.split(' ')[0];
          const updatedAt = dayjs().utc().tz(config.timezone).toDate()
      ...
      await jobBLL.update({ where: { _id }, data: { $set: { status: constant.JOB_STATUS.FAIL, duration } } });
    } catch (e) {
      console.log(e.message);
      await jobBLL.update({ where: { _id }, data: { $set: { status: constant.JOB_STATUS.FAIL } } });
    } finally {
